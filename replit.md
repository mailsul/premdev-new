...
### 6. Fitur baru (06 Mei 2026) — Editor features batch 2-8

**Files diubah:** `apps/web/src/pages/Editor.tsx`, `apps/web/src/components/AIChat.tsx`, `apps/api/src/routes/workspaces.ts`, `apps/api/src/lib/db.ts`

1. **Auto-save** — sudah ada (1500ms debounce), tidak diubah.
2. **AI multi-file attach** — ikon Paperclip di toolbar AI Chat. Klik → modal `AIFilePicker` (flat file tree, keyboard nav ↑↓/Enter/Esc). File terpilih diambil via `GET /workspaces/:id/files/raw?path=…`, disuntikkan sebagai `<attached-file>` block ke awal pesan. Chips biru tampil di atas textarea; klik × untuk hapus. State: `attachedFiles[]`, `attachPickerOpen`.
3. **Find & Replace workspace-wide** — shortcut `Ctrl+Shift+H` dan tombol toolbar. Modal `WorkspaceReplace`: dua input (find/replace), checkbox regex, tombol Search + Replace All. Search via `POST /workspaces/:id/search`; replace via raw-read + string replace + `PUT /workspaces/:id/files` per file. Klik hasil → buka file di editor.
4. **Drag-and-drop upload** — `FileTree` container punya `onDragEnter/Leave/Over/Drop`. Drop file dari OS → `uploadFiles(files)`. Visual highlight biru saat drag hover.
5. **Shortcut reference modal** — shortcut `Ctrl+?` dan tombol toolbar. Modal `ShortcutModal` dengan 3 grup: Editor, AI Chat, Terminal. Semua shortcut PremDev terdaftar.
6. **Persistent AI chat history (server-side)** — backend: tabel `chat_history` (SQLite), endpoint `GET/PUT/DELETE /workspaces/:id/chat-history?tab=…`. Frontend: save 5s debounce ke server saat `msgs` berubah; load dari server saat mount jika localStorage kosong.
7. **Vim mode toggle** — tombol "VIM" di toolbar editor. State `vimMode` (persisted ke localStorage). Aktif: `cursorStyle: "block"`, `lineNumbers: "relative"`.

**Cara deploy ke VPS:** soft deploy biasa (`sudo git pull && sudo docker compose pull app && sudo docker compose up -d app`). Migration DB (`chat_history` table) dibuat otomatis saat app startup via `db.ts`.

---

### 7. Workflow Replit → GitHub → VPS (untuk user)
...
### 8. Perubahan terbaru (07 Mei 2026)

**Workspace error page → HTML cantik dalam bahasa Inggris** (`apps/api/src/routes/proxy.ts`):
- Sebelumnya: plain text "Workspace not running" / "Workspace tidak berjalan..."
- Sekarang: halaman HTML dark-mode bergaya sama dengan landing page, menampilkan ikon, judul, dan pesan deskriptif dalam bahasa Inggris
- Pesan berbeda tergantung kondisi: stopped → suruh klik Run; ECONNREFUSED → info tentang port binding / headless process; DNS fail → suruh restart; name collision → HTTP 409 halaman merah

**OpenRouter live model list** (`apps/api/src/lib/ai-providers.ts`, `apps/api/src/routes/ai.ts`):
- Tambah `fetchOpenRouterModels()` — fetch `https://openrouter.ai/api/v1/models`, cache 10 menit (sama pola dengan Google & SnifoxAI)
- Model gratis (`:free` suffix) diurutkan ke atas dalam dropdown
- `/api/ai/providers` endpoint sekarang pakai live list OpenRouter (ratusan model + free tier) alih-alih static list

**Multi-domain support — OTOMATIS via Admin panel** (`apps/api/src/lib/caddy.ts`, `apps/api/src/routes/admin.ts`, `infra/Caddyfile.tmpl`, `infra/docker-compose.prod.yml`):
- `apps/api/src/lib/caddy.ts` — helper baru: `writeDomainSnippet()`, `deleteDomainSnippet()`, `reloadCaddy()` via dockerode exec
- Admin domain endpoints (`POST /domains`, `PATCH /domains/:name/toggle`, `DELETE /domains/:name`) sekarang otomatis write/delete snippet + reload Caddy setelah setiap perubahan
- Snippet files: `{DATA_DIR}/caddy/extra/<domain>.caddy` (app) = `/opt/premdev/data/caddy/extra/<domain>.caddy` (host)
- **SETUP SATU KALI di VPS** (hanya perlu dilakukan sekali):
  1. Tambah `import /etc/caddy/extra/*.caddy` di Caddyfile VPS (setelah blok global `{}`)
  2. Edit docker-compose.yml — tambah di volumes caddy: `- /opt/premdev/data/caddy/extra:/etc/caddy/extra:ro`
  3. `sudo mkdir -p /opt/premdev/data/caddy/extra && sudo docker compose up -d caddy`
- **Setelah setup satu kali**: tambah domain via Admin panel → snippet otomatis dibuat + Caddy reload + SSL cert diminta → done. Hapus domain → snippet terhapus + Caddy reload otomatis.

**Cara deploy ke VPS (soft deploy):**
```bash
ssh root@flixprem.org
cd /opt/premdev
sudo git pull origin main
...
sudo docker compose up -d app
```
Untuk domain baru, juga restart caddy: `sudo docker compose up -d caddy`
...
## Multi-domain + UI improvements (Mei 2026)

### ActionCard collapse (AIChat.tsx)
- Autonomous (otonom) cards start collapsed — header always visible, click to toggle body.
- Collapsed header shows status badge (spinner / OK / Error) at the right.
- Error state auto-expands even if collapsed.
- Manual (approval-needed) cards always stay open.
- `ChevronDown` + `Loader2` added to lucide imports.

### Admin RPM display (Admin.tsx)
- `SettingRow` now shows `= X req/menit` hint for any field whose key contains `RefillPerSec`. Updates live as user types.

### "[Auto pilih...]" filter (AIChat.tsx)
- `filterStreamAnnotations()` strips `[Auto pilih ...]` / `[Auto: ...]` lines from both streaming accumulator points.

### Multi-domain support (Mei 2026)
Admin can register additional base domains pointing to this VPS. Users pick a domain when setting a custom subdomain.

**Backend:**
- `apps/api/src/lib/db.ts`: new `custom_domains` table `(id, name, active, added_at)`. New `workspaces.custom_domain` column (NULL = PRIMARY_DOMAIN). `DbCustomDomain` type. `getActiveDomains()` export. `workspaceUrl()` now accepts optional `customDomain` param. `workspaceToPublic()` exposes `customDomain`.
- `apps/api/src/routes/admin.ts`: `GET/POST /admin/domains`, `PATCH /admin/domains/:name/toggle`, `DELETE /admin/domains/:name`.
- `apps/api/src/routes/workspaces.ts`: `GET /workspaces/domains` (public list for logged-in users). `SubdomainBody` gains optional `domain` field. `PUT /:id/subdomain` validates domain against DB and saves `custom_domain`. Clear path also clears `custom_domain`.
- `apps/api/src/routes/proxy.ts`: `resolveSubdomain(sub, incomingDomain)` — domain-aware lookup; verifies `custom_domain` matches incoming domain; auto-form only for PRIMARY_DOMAIN. `targetForHost` checks all active custom domains via `getActiveDomains()`.
- `apps/web/src/lib/api.ts`: added `API.patch()` method.

**Frontend:**
- `apps/web/src/pages/Admin.tsx`: new "Domains" tab. `DomainsSection` component — lists PRIMARY + custom domains, toggle active/inactive, delete, add new with Cloudflare instruction card.
- `apps/web/src/pages/Editor.tsx`: `Workspace` type gains `customDomain`. `SubdomainPanel` fetches `/workspaces/domains`, shows domain dropdown when >1 domain available, live preview URL uses chosen domain, save passes `domain` field.
...
## Persistent AI memory / cross-session learning (Mei 2026)
AI agent now remembers user preferences and project facts across sessions via `.premdev/memory.md` per workspace.

**How it works:**
1. User clicks "Memori" button in AI toolbar → calls `POST /api/ai/memory/update`
2. Backend runs a special "memory curator" AI call (same provider/model as selected) that extracts learnings from the current conversation
3. Learnings written to `.premdev/memory.md` inside the workspace (auto-creates `.premdev/` dir if needed)
4. Every future chat turn injects both `instructions.md` (user-authored) AND `memory.md` (AI-generated) into the system prompt
5. System prompt instructs the AI to silently use both without quoting them back to user
6. Auto-suggest banner appears after 15 messages, dismissible

**Files changed:**
- `apps/api/src/lib/ai-context.ts`: split `loadProjectMemory` (instructions.md only) + new `loadAIMemory` (memory.md, 6 KB limit) + `writeAIMemory` + shared `readPremdevFile` helper
- `apps/api/src/routes/ai.ts`: imports `loadAIMemory`/`writeAIMemory`; injects `aiMemoryBlock` separately labeled in system prompt; new `POST /memory/update` route with curator prompt
- `apps/api/src/lib/ai-prompt.ts`: `PERSISTENT MEMORY` rule added to system prompt so AI knows to silently use the injected memory sections
- `apps/web/src/components/AIChat.tsx`: `Brain` icon added; `savingMemory`/`memorySaved`/`memoryDismissed` states; `saveMemory()` async function; "Memori" pill button in toolbar (spinner/check/brain icon); auto-suggest banner at 15+ messages
