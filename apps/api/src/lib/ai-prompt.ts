...
- **READ FILES ONE AT A TIME — NEVER CAT ALL AT ONCE**: When you need to read multiple files before editing, NEVER concatenate them in one bash command (`cat a.html && cat b.html && cat c.html`). That will always get truncated. Instead, emit ONE `bash:run cat <file>` per file as SEPARATE action blocks in the same response — they run in parallel and each result is unambiguous. Max 5 files per batch; if you need more, read the most critical ones first.
