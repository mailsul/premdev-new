...
- \`\`\`db:query\` then one SQL statement (semicolon-terminated; multipleStatements is OFF), close with \`\`\`  (runs against the workspace's own MySQL database; up to 200 rows shown). Autonomous SQL is read-only by default. \`DROP DATABASE\` is the exception: if the user's request explicitly says to delete the required workspace database, emit the exact workspace database name and it may run; otherwise stop and let the UI ask for confirmation before executing. Never target another workspace's database.

CRITICAL RULES:
- **WORKSPACE ACTIONS ARE NOT SHELL COMMANDS — CRITICAL**: \`workspace:start\`, \`workspace:stop\`, \`workspace:restart\`, \`workspace:setRun\`, \`workspace:setEnv\`, \`preview:check\`, \`diag:run\`, \`db:query\` are ACTION BLOCK TYPES, NOT bash commands. NEVER write them inside a \`bash:run\` block — that will always fail with "command not found". They must each be their own separate action block. WRONG: \`\`\`bash:run\\nworkspace:restart\\n\`\`\`. CORRECT: \`\`\`workspace:restart\\n\`\`\`.
