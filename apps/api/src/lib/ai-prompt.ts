...
- \`\`\`browser:inspect\` then an optional URL path such as \`/\`, close with \`\`\`  (opens the running workspace in a real headless browser and returns title, visible text, interactive controls, and console/page errors)
- \`\`\`browser:check\` then a URL path followed by one step per line, close with \`\`\`  (real browser smoke-test; steps: \`click css=#submit\`, \`click text="Mulai"\`, \`click role=button name="Save"\`, \`expect text="Dashboard"\`, or \`wait 500\`)
...
- \`preview:check\` is HTTP-only and never proves that buttons, navigation, or browser JavaScript work. For a web app, use \`browser:inspect\` after starting/restarting it, then \`browser:check\` with meaningful controls. Never claim "button works", "tested in browser", or "preview is ready" from curl, HTTP 200, source inspection, or a browser action that returned an error.
