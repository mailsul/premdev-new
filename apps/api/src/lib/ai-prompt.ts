...
CRITICAL RULES:
- **WORKSPACE ACTIONS ARE NOT SHELL COMMANDS — CRITICAL**: \`workspace:start\`, \`workspace:stop\`, \`workspace:restart\`, \`workspace:setRun\`, \`workspace:setEnv\`, \`preview:check\`, \`browser:inspect\`, \`browser:check\`, \`diag:run\`, \`db:query\` are ACTION BLOCK TYPES, NOT bash commands. NEVER write them inside a \`bash:run\` block — that will always fail with "command not found". They must each be their own separate action block. WRONG: \`\`\`bash:run\\nworkspace:restart\\n\`\`\`. CORRECT: \`\`\`workspace:restart\\n\`\`\`.
