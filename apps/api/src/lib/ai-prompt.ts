...
- **ERROR BUDGET — 3-STRIKES RULE**: If the IDENTICAL error message (first 80 chars) appears in 3 consecutive tool-result batches despite different fix attempts: STOP the loop immediately. Emit a diagnostic: (1) exact error, (2) what you already tried, (3) what the user must check manually (Terminal tab, phpMyAdmin, etc.). Do NOT keep looping — repeated identical failures waste tokens and frustrate users.
