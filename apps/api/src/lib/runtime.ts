...
export async function startContainer(opts: {
  ...
  // Bash snippet that monkey-patches Python web frameworks so they always bind
  // to 0.0.0.0 even when the user's code has a hardcoded host (or no host).
  // Technique: inject a sitecustomize.py via PYTHONPATH — Python imports it
  // before any user code, so Flask/uvicorn/Bottle/etc. get patched silently.
  // We also patch Rack (Ruby) so Sinatra / Roda / plain rack apps bind to
  // 0.0.0.0 when the HOST env var is set.
  const HOST_FIX_SNIPPET = `\
mkdir -p /tmp/.pd_site
cat > /tmp/.pd_site/sitecustomize.py << 'PYEOF'
import os as _o
# Flask / Werkzeug
try:
    import flask as _f
    _fr = _f.Flask.run
    def _fp(self, host=None, port=None, **k): _fr(self, host or _o.environ.get('HOST','0.0.0.0'), port, **k)
    _f.Flask.run = _fp
...
try:
    import bottle as _b
    _br = _b.run
    def _bp(host=None, port=None, **k): _br(host=host or _o.environ.get('HOST','0.0.0.0'), port=port, **k)
    _b.run = _bp
except ImportError: pass
# FastAPI / Starlette via uvicorn.run()
try:
    import uvicorn as _u
    _ur = _u.run
    def _up(*a, host=None, port=None, **k): _ur(*a, host=host or _o.environ.get('HOST','0.0.0.0'), port=port, **k)
    _u.run = _up
except ImportError: pass
PYEOF
export PYTHONPATH=/tmp/.pd_site\${PYTHONPATH:+:\$PYTHONPATH}
# Rack (Ruby) — patch TCPServer so Sinatra/Roda bind to 0.0.0.0
if command -v ruby >/dev/null 2>&1; then
  export RUBYOPT="-r/tmp/.pd_site/rackfix"
  cat > /tmp/.pd_site/rackfix.rb << 'RBEOF'
require 'socket' rescue nil
if defined?(Rack)
  Rack::Server.class_eval do
    alias_method :_pd_start, :start rescue nil
    def start(after_fork=nil)
      options[:Host] ||= ENV['HOST'] || '0.0.0.0'
      _pd_start(after_fork)
    end rescue nil
  end rescue nil
end
RBEOF
...
cd /workspace && exec bash -lc ${JSON.stringify(opts.runCommand)}`
  ...
  const runBinds: string[] = [`${wsHostDir}:/workspace`, ...userHomeBinds(opts.workspaceId)];
  let runHasSock = false;
  try {
    if (fs.existsSync(runDockerSock)) {
      runBinds.push(`${runDockerSock}:/var/run/docker.sock`);
      runHasSock = true;
    }
  } catch {}

  // Prefix chmod socket sebelum run command user berjalan.
  const sockFix = runHasSock ? "sudo chmod 666 /var/run/docker.sock 2>/dev/null || true; " : "";
  const finalRunCmd = runCmd ? sockFix + runCmd : null;
  ...
  const container = await docker.createContainer({
    ...
    Cmd: finalRunCmd
      ? ["bash", "-lc", finalRunCmd]
      : ["bash", "-lc", sockFix + "cd /workspace && exec bash -l"],
    HostConfig: {
      ...
      // no-new-privileges dimatikan supaya sudo chmod socket bisa jalan.
    },
    Labels: {
...
export async function ensureShellContainer(workspaceId: string): Promise<string> {
  ...
  // Merge order: DB env_vars (base) → .premdev env (wins on conflict),
  // matching the same priority as resolveEnvVars in the run container.
  ...
  // Also read from .premdev config (where SecretsPanel & AI setEnv store vars).
  try {
    const wsDir = workspacePath(workspaceId);
    const cfg = readWorkspaceConfig(wsDir);
  ...
  // Mount docker socket kalau tersedia di host, biar user bisa jalankan
  // `docker` dan `docker compose` langsung dari terminal workspace.
  // chmod 666 di startup script supaya premdev user (UID 1000) bisa akses
  // socket tanpa harus join docker group di host.
  const dockerSock = config.DOCKER_SOCKET;
  const dockerBinds: string[] = [`${wsHostDir}:/workspace`, ...userHomeBinds(workspaceId)];
  let hasSock = false;
  try {
    const fs2 = await import("node:fs");
    if (fs2.existsSync(dockerSock)) {
      dockerBinds.push(`${dockerSock}:/var/run/docker.sock`);
      hasSock = true;
    }
  } catch {}

  // Startup: fix socket permission lalu sleep infinity.
  // no-new-privileges SENGAJA tidak dipakai di shell container supaya
  // `sudo` bisa jalan (premdev sudah punya NOPASSWD di sudoers).
  const startupCmd = hasSock
    ? "chmod 666 /var/run/docker.sock 2>/dev/null || true; exec sleep infinity"
    : "exec sleep infinity";
  ...
  const container = await docker.createContainer({
    ...
    Tty: true,
    OpenStdin: true,
    // Jalankan startup sebagai root supaya chmod docker socket bisa jalan.
    // Terminal exec (`docker exec -u premdev`) tetap masuk sebagai premdev.
    User: "root",
    Cmd: ["bash", "-c", startupCmd],
    WorkingDir: "/workspace",
    Env: shellEnv,
    HostConfig: {
      NetworkMode: config.DOCKER_NETWORK,
      ...
      CapDrop: ["ALL"],
      CapAdd: ["CHOWN", "DAC_OVERRIDE", "FOWNER", "SETUID", "SETGID"],
      // SecurityOpt no-new-privileges sengaja TIDAK dipakai di shell container
      // supaya sudo bisa jalan. Run container (pw_*) tetap pakai no-new-privileges.
      RestartPolicy: { Name: "no" },
    },
