...
export const fileRoutes: FastifyPluginAsync = async (app) => {
  ...
  // Serve a file as raw binary — used by frontend to preview images.
  ...
  app.get("/:id/files/raw", async (req, reply) => {
    ...
    const stats = fs.statSync(abs);
    if (stats.size > 20 * 1024 * 1024) return reply.code(413).send({ error: "File too large" });
    const ext = path.extname(q.path).toLowerCase();
    const mime = IMAGE_MIME[ext] ?? "application/octet-stream";
    const buf = fs.readFileSync(abs);
    reply.header("Content-Type", mime);
    if (q.download === "1") {
      const fname = path.basename(q.path).replace(/[^\w.\-]/g, "_") || "file";
  ...
  app.post("/:id/files/patch", async (req, reply) => {
    ...
    // Pass 1: CRLF → LF + trim trailing spaces per line.
    // Pass 2 (fallback): also strip ALL leading whitespace per line
    //   (catches tab-vs-space indentation mismatches — the #1 cause of
    //   "Find string not found" in AI-generated patches).
    function normalizeWs(s: string): string {
      return s
        .replace(/\r\n/g, "\n")
        .replace(/\r/g, "\n")
        .replace(/[^\S\n]+$/mg, "");    // trim trailing spaces per line
    }
    function normalizeIndent(s: string): string {
      return normalizeWs(s).replace(/^[^\S\n]+/mg, ""); // also strip leading ws
    }
    ...
    let workingFind = body.find;
    let workingReplace = body.replace;
    let fuzzy = false;

    let occurrences = countOccurrences(original, body.find);
    if (occurrences === 0) {
      // Pass 1: normalize trailing whitespace + CRLF.
      const normOrig = normalizeWs(original);
      const normFind = normalizeWs(body.find);
      const normOccurrences = countOccurrences(normOrig, normFind);
      if (normOccurrences > 0) {
        workingOriginal = normOrig;
        workingFind = normFind;
        workingReplace = normalizeWs(body.replace);
        occurrences = normOccurrences;
        fuzzy = true;
      }
    }
    if (occurrences === 0) {
      // Pass 2: strip ALL leading whitespace per line (tab vs spaces).
      const indOrig = normalizeIndent(original);
      const indFind = normalizeIndent(body.find);
      const indOccurrences = countOccurrences(indOrig, indFind);
      if (indOccurrences > 0) {
        workingOriginal = indOrig;
        workingFind = indFind;
        workingReplace = normalizeIndent(body.replace);
        occurrences = indOccurrences;
        fuzzy = true;
      }
    }

    if (occurrences === 0) {
      // Still not found — return a large snippet so the AI can self-correct
      // WITHOUT re-reading the file (matches the PATCH SELF-RECOVERY rule).
      const findFirstLine = body.find.trim().split("\n")[0].trim().slice(0, 80);
      const fileLines = original.split("\n");
      // Find the nearest matching line using char-overlap scoring.
      let nearestIdx = -1;
      let bestScore = 0;
      for (let i = 0; i < fileLines.length; i++) {
        ...
        if (!l) continue;
        let shared = 0;
        for (let c = 0; c < Math.min(l.length, findFirstLine.length); c++) {
          if (l[c] === findFirstLine[c]) shared++;
        }
        if (shared > bestScore) { bestScore = shared; nearestIdx = i; }
      }
      // Show 10 lines before + 30 lines after (wide enough for self-correction).
      const snippetStart = Math.max(0, nearestIdx - 10);
      const snippetEnd   = Math.min(fileLines.length, nearestIdx + 30);
      const snippet = nearestIdx >= 0
        ? fileLines.slice(snippetStart, snippetEnd).map((l, i) => `${snippetStart + i + 1}: ${l}`).join("\n")
        : fileLines.slice(0, 40).map((l, i) => `${i + 1}: ${l}`).join("\n");
      return reply.code(422).send({
        error: "Find string not found",
        hint: `Your find text does not exist verbatim in ${body.path} (tried exact, trailing-ws, and indentation-stripped matches). ` +
          `The actual file content near your target:\n\n${snippet}\n\n` +
          `Copy the EXACT lines from the numbered snippet above (with their real indentation) into a corrected patch: block. DO NOT emit bash:run cat — use the snippet above directly.`,
      });
    }
    ...
    const updated = body.replaceAll
      ? workingOriginal.split(workingFind).join(workingReplace)
      : workingOriginal.replace(workingFind, workingReplace);
