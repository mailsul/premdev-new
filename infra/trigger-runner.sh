...
#
# Supported actions: backup, restore, refresh (index), delete (snapshot), cleanup
# ============================================================================
set -Eeuo pipefail
  ...
  case "$action" in
    ...
    delete)
      ...
      if [[ -z "$snapshot" ]]; then
        ...
        status="error"; output="R2_BUCKET not configured"
      elif ! echo "$snapshot" | grep -qE '^(daily|weekly)/[0-9]{8}-[0-9]{6}$'; then
        status="error"; output="invalid snapshot path: $snapshot"
      else
        if rclone purge "${RC_REMOTE}:${BUCKET}/${snapshot}/" >/tmp/_pdj.log 2>&1; then
          output="Deleted ${snapshot}"
          /usr/local/sbin/premdev-refresh-index >>/tmp/_pdj.log 2>&1 || true
        else
          exit_code=$?
          status="error"
          output="$(tail -c 2000 /tmp/_pdj.log)"
